...
export async function POST(req: Request) {
  try {
    ...
    if (!name?.trim()) return new NextResponse("Skill name required", { status: 400 });
    ...
    // Create the user-skill relation (ignore duplicates)
    ...
    console.error("[PROFILE_SKILLS_POST]", error);
